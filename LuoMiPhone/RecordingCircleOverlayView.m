...
- (id)initWithFrame:(CGRect)frame strokeWidth:(CGFloat)strokeWidth insets:(UIEdgeInsets)insets
{
    if (self = [super initWithFrame:frame])
    {
        ...
        self.arcCenter = CGPointMake( CGRectGetMidX(frame),CGRectGetMidY(frame));
        CGFloat radius = 80;
        
        self.circlePath = [UIBezierPath bezierPathWithArcCenter:_arcCenter
                                                         radius:radius
                                                     startAngle:M_PI
                                                       endAngle:-M_PI
                                                      clockwise:NO];
...
- (void)addBackgroundLayer
{
    ...
    layer.bounds = CGRectMake(0, 0, 120, 40);
    layer.string = @"按住任何地方拍";
    layer.fontSize = 17;
