...
-(void) prepareLayout {
  ...
  self.spacingBetweenCollectionView = self.interItemSpacing / 3;
  self.lastYValueForColumn = [NSMutableDictionary dictionary];
  CGFloat currentColumn = 0;
  CGFloat fullWidth = self.collectionView.frame.size.width;
  CGFloat availableSpaceExcludingPadding = fullWidth - (self.interItemSpacing * 2 + (self.numberOfColumns - 1) * self.spacingBetweenCollectionView);
  ...
  for(NSInteger section = 0; section < numSections; section++)  {
    ...
    for(NSInteger item = 0; item < numItems; item++){
      ...
      CGFloat x = self.interItemSpacing + (self.spacingBetweenCollectionView + itemWidth) * currentColumn;
      ...
      y+= height;
      y += self.spacingBetweenCollectionView;
